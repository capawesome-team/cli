...
describe('apps-import', () => {
  ...
  it('should set the next build number for apps with a latest build number', async () => {
    await writeExportFile([
      {
        folder: 'My App-6668c18c',
        files: {
          'app-detail.json': { id: '6668c18c', name: 'My App', appType: 'capacitor', latestBuildNumber: 142 },
        },
      },
    ]);
    ...
    const scope = nock(DEFAULT_API_BASE_URL)
      ...
      .patch('/v1/apps/app-1', { nextAppBuildNumber: 143 })
      .reply(200, { id: 'app-1', name: 'My App', type: 'capacitor' })
      .get('/v1/apps/app-1/channels')
      .reply(200, []);
    ...
    expect(scope.isDone()).toBe(true);
    const output = getJsonOutput();
    expect(output.apps[0].notes).toContainEqual(expect.stringContaining('`143`'));
  });
